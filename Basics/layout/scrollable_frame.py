...
class ListFrame(ttk.Frame):
    ...
    def create_item(self, index, item):
        ...
        # widgets
        ttk.Label(frame, text=f'#{index}').grid(column=0, row=0)
        ttk.Label(frame, text=f'{item[0]}').grid(column=1, row=0)
        ttk.Button(frame, text=f'{item[1]}').grid(
            column=2, row=0, columnspan=3, sticky='nswe')
...
text_list = [('label', 'button'), ('thing', 'click'),
             ('third', 'something'), ('label 1', 'label 2'),
             ('forth', 'i do not know'), ('just another label', 'just another button')]
