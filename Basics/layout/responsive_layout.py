...
class App(tk.Tk):
    def __init__(self, start_size: tuple):
        super().__init__()
    ...
    def create_medium_layout(self):
        ...
        ttk.Label(self.frame, text='Label 1', background='red').grid(
            column=0, row=0, sticky='nswe', padx=10, pady=10)
        ttk.Label(self.frame, text='Label 2', background='green').grid(
            column=1, row=0, sticky='nswe', padx=10, pady=10)
        ttk.Label(self.frame, text='Label 3', background='blue').grid(
            column=0, row=1, sticky='nswe', padx=10, pady=10)
        ttk.Label(self.frame, text='Label 4', background='yellow').grid(
            column=1, row=1, sticky='nswe', padx=10, pady=10)
...
class SizeNotifier:
    ...
    def check_size(self, event):
        # checks if the widget calling configure is the main window
        if event.widget == self.window:
            # the size_dict MUST be sorted
